...
I noticed something curious in looking through my own Draftbacks. Places where things got tricky and I went back and forth within a single sentence played out for several seconds, long enough to read them, but major chunks of text would appear or disappear in the blink of an eye during playback. Why? Because every edit – whether a word or a paragraph – is treated as one change, and given equal screen time. Because it doesn't play back in real time (or else we'd be watching it for days or weeks!), Draftback doesn't seem to *pause.* This is great for identifying stuckpoints, which can certainly be helpful to reflect on – but it actually makes it hard to see the breakthroughs, which are just as important, if not more. 
